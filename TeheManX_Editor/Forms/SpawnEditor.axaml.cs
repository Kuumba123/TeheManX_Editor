...
public partial class SpawnEditor : UserControl
{
    ...
    public void CollectData()
    {
        ...
        if (Const.Id != Const.GameId.MegaManX)
        {
            silkShotInt.IsVisible = true;
            silkShotTxt.IsVisible = true;
            megaFlipInt.IsVisible = true;
            megaFlipTxt.IsVisible = true;

            if (Const.Id == Const.GameId.MegaManX3)
            {
                scrollInt.IsVisible = true;
                scrollTypeText.IsVisible = true;
            }
            else
            {
                scrollInt.IsVisible = false;
                scrollTypeText.IsVisible = false;
            }
        }
        else
        {
            silkShotInt.IsVisible = false;
            silkShotTxt.IsVisible = false;
            megaFlipInt.IsVisible = false;
            megaFlipTxt.IsVisible = false;
            scrollInt.IsVisible = false;
            scrollTypeText.IsVisible = false;
        }
    }
    ...
    {
        Checkpoint point = Checkpoints[Level.Id][spawnInt.Value];
    ...
    #region Events
    private void spawnInt_ValueChanged(object sender, int e)
    {
    ...
    private void objectTileInt_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void backgroundTileInt_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void backgroundPalInt_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void silkShotInt_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void megaIntX_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void megaIntY_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void camIntX_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void camIntY_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void bg2IntX_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void bg2IntY_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void camBorderIntL_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void camBorderIntR_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void camBorderIntT_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void camBorderIntB_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void bg2IntBaseX_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void bg2IntBaseY_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void wramIntInt_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void megaFlipInt_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private void scrollInt_ValueChanged(object sender, int e)
    {
        if (SNES.rom == null || Level.Id >= Const.PlayableLevelsCount || supressInts)
            return;
    ...
    private async void EditCheckpointBtn_Click(object sender, RoutedEventArgs e) // Configure Max Checkpoints in the current stage
    {
        if (Level.Id >= Const.PlayableLevelsCount || (Const.Id == Const.GameId.MegaManX3 && Level.Id > 0xE))
        {
            await MessageBox.Show(MainWindow.window, "This level does not support checkpoints.", "ERROR");
            return;
        }
        ...
        confirmBtn.Click += async (se, ev) =>
        {
            ...
            {
                await MessageBox.Show(MainWindow.window, $"The total amount of checkpoints across all levels cannot exceed {Const.MaxTotalCheckpoints}.\nCurrent Total: {total}", "ERROR");
                return;
            }
            ...
            SNES.edit = true;
            await MessageBox.Show(MainWindow.window, "Checkpoint configuration updated successfully!");
            SetSpawnSettings();
            MainWindow.window.enemyE.DrawLayout();
            window.Close();
        };
