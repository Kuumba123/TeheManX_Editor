...
public partial class ProjectWindow : Window
{
    ...
    private async void expandBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            await MessageBox.Show(this, "You must exit 16x16 mode if you want to enable the 4MB expansion!");
            return;
        }
        if (SNES.rom.Length >= 0x400000 && Encoding.ASCII.GetString(SNES.rom, 0x3FFFF0, 6) == "POGYOU")
        {
            if (await MessageBox.Show(this, "You already have the expansion patch. Are you sure you want to apply it again?", "", MessageBoxButton.YesNo))
                return;
        }
        ...
        await MessageBox.Show(this, $"4MB expansion complete! Every bank after 0x{((dumpOffset / 0x8000) | (addrMask >> 16)):X} is availble for use!");
        ...
        if (Const.Id == Const.GameId.MegaManX && await MessageBox.Show(this, "Would apply the patch that disables the cart size checks?", "", MessageBoxButton.YesNo))
        {
            ...
            await MessageBox.Show(this, "Cart size check patch applied!");
        }
    }
    private async void expansionHelpBtn_Click(object sender, RoutedEventArgs e)
    {
        HelpWindow helpWindow = new HelpWindow(6);
        await helpWindow.ShowDialog(this);
    }
    private async void projectTrackPatchBtn_Click(object sender, RoutedEventArgs e)
    {
        await MessageBox.Show(this, "The patch has not been fully complete at the time of this editor version\nbut will be completed soon!");
    }
    private async void projectTrackHelpBtn_Click(object sender, RoutedEventArgs e)
    {
        HelpWindow helpWindow = new HelpWindow(7);
        await helpWindow.ShowDialog(this);
    }
    ...
    private void Int_ValueChanged(object sender, int e)
    {
        if (!enable)
            return;
        edited = true;
