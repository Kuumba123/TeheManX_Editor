...
public partial class ToolsWindow : Window
{
     ...
     */
    private async void MMX1_DecompressClick(object sender, RoutedEventArgs e)
    {
        ...
        if (file != null)
        {
            byte[] rom = await File.ReadAllBytesAsync(file.Path.LocalPath);
            ...
            if (folder != null)
            {
                ...
                {
                    await MessageBox.Show(this, "Invalid Rom!\nDid not find correct Cart Name.", "ERROR");
                    return;
                }
                for (int s = 0; s < Const.MegaManX.CompressedTilesAmount; s++)
                {
                    ...
                    try
                    {
                        ...
                        await File.WriteAllBytesAsync($"{folder.Path.LocalPath}\\Tiles_{s:X2}.bin", data);
                    }
                    catch (Exception ex)
                    {
                        await MessageBox.Show(this, $"Error happened when decompress - {s:X}" + " Tile Graphics" + ex.Message + "\nCorrupted ROM ?", "ERROR");
                        return;
                    }
                }
                await MessageBox.Show(this, "Tiles Data Decompressed!");
            }
        }
    }
    private async void MMX1_CompressClick(object sender, RoutedEventArgs e)
    {
        IStorageProvider storageProvider = StorageProvider;
        ...
        if (file != null)
        {
            ...
            if (saveFile != null)
            {
                ...
                try
                {
                    byte[] inputData = File.ReadAllBytes(file.Path.LocalPath);
                    ...
                    bw.Close();
                    ms.Close();
                    await File.WriteAllBytesAsync(saveFile.Path.LocalPath, ms.ToArray());
                }
                catch (Exception ex)
                {
                ...
                //Done
                await MessageBox.Show(this, "MegaMan X Tile Data Compressed!");
            }
        }
    }
    private async void MMX1_ExtractCompressClick(object sender, RoutedEventArgs e)
    {
        IStorageProvider storageProvider = StorageProvider;
        ...
        if (file != null)
        {
            ...
            IReadOnlyList<IStorageFolder> folders = await storageProvider.OpenFolderPickerAsync(new FolderPickerOpenOptions
            {
                Title = "Select Save Location for Compressed Tiles",
                AllowMultiple = false
            });
            IStorageFolder? folder = folders.FirstOrDefault();
            ...
            if (folder != null)
            {
                ...
                for (int s = 0; s < Const.MegaManX.CompressedTilesAmount; s++)
                {
                    ...
                    try
                    {
                        ...
                        bw.Close();
                        await File.WriteAllBytesAsync($"{folder.Path.LocalPath}\\CompressedTiles_{s:X2}.bin", ms.ToArray());
                        ms.Close();
                    }
                    catch (Exception ex)
                    {
                        await MessageBox.Show(this, $"Error happened when extract - {s:X}" + " compressed Tile Graphics" + ex.Message + "\nCorrupted ROM ?", "ERROR");
                        return;
                    }
                }
                await MessageBox.Show(this, "Compressed Tiles Data Extracted!");
            }
        }
    }
    private async void MMX1_PaletteEditorClick(object sender, RoutedEventArgs e)
    {
        IStorageProvider storageProvider = StorageProvider;
        ...
        if (file != null)
        {
            ...
            {
                await MessageBox.Show(this, "Invalid Game");
                return;
            }
            ExternalPaletteWindow paletteWindow = new ExternalPaletteWindow(rom, gameId, filePath);
    ...
    private async void ConfigureGameSettings_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (file != null)
        {
            ...
            GameSettingsWindow gameSettings = new GameSettingsWindow(fileName, rom, (int)gameId, (int)version);
            await gameSettings.ShowDialog(this);
        }
    }
    /*
    *  MegaMan X2 Tools Events
    */
    private async void MMX2_DecompressClick(object sender, RoutedEventArgs e)
    {
        IReadOnlyList<IStorageFile> result = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
        {
            Title = "Open an MegaMan X1-3 SFC File",
            ...
            if (folder != null)
            {
                ...
                for (int s = 0; s < count; s++)
                {
                    ...
                    try
                    {
                        ...
                        File.WriteAllBytes($"{folder.Path.LocalPath}\\Tiles_{s:X2}.bin", data);
                    }
                    catch (Exception ex)
                    {
                        await MessageBox.Show(this, $"Error happened when decompress - {s:X}" + " Tile Graphics" + ex.Message + "\nCorrupted ROM ?", "ERROR");
    ...
    private async void MMX2_CompressClick(object sender, RoutedEventArgs e)
    {
        IStorageProvider storageProvider = StorageProvider;
        ...
        if (file != null)
        {
            ...
            if (saveFile != null)
            {
                ...
                try
                {
                    ...
                    await File.WriteAllBytesAsync(saveFile.Path.LocalPath, compressedBytes.ToArray());
                }
                catch (Exception ex)
                {
                    await MessageBox.Show(this, ex.Message, "ERROR");
                ...
                //Done
                await MessageBox.Show(this, "MegaMan X2/MegaMan X3 Tile Data Compressed!");
            }
        }
    }
    private async void MMX2_ExtractCompressClick(object sender, RoutedEventArgs e)
    {
        IReadOnlyList<IStorageFile> result = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
        {
            Title = "Open an MegaMan X1-3 SFC File",
            ...
            if (folder != null)
            {
                ...
                for (int s = 0; s < count; s++)
                {
                    ...
                    try
                    {
                        ...
                        bw.Close();
                        File.WriteAllBytes($"{folder.Path.LocalPath}\\CompressedTiles_{s:X2}.bin", ms.ToArray());
                        ms.Close();
                    }
                    catch (Exception ex)
                    {
